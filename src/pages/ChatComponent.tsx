...
const ChatComponent: React.FC<{}> = () => {
    ...
    useEffect(() => {
        ...
        const onMessageReceived = (user: string, receivedMessage: string) => {
            setMessages((prevMessages) => [...prevMessages, `${user}: ${receivedMessage}`]);
    ...
    const sendMessage = () => {
        if (message.trim() !== "") {  // To prevent empty message sending
    ...
    return (
        <div className={styles.chatComponent}>
            ...
            <Input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') sendMessage(); }}  // Trigger send on Enter
            />
            <Button className={styles.sendMessageButton} type="primary" onClick={sendMessage}>Send Message</Button>
        </div>
    );
};
